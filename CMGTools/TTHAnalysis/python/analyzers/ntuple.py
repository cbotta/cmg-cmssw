...
def bookLepton( tree, pName, isMC=False ):
    ...
    var(tree, '{pName}_tightFakeId'.format(pName=pName))
    var(tree, '{pName}_looseFakeId'.format(pName=pName))
...
def fillLepton( tree, pName, lepton ):
    ...
    fill(tree, '{pName}_sip3d'.format(pName=pName), lepton.sip3D() if abs(lepton.pdgId())==11 or lepton.sourcePtr().innerTrack().isNonnull() else -9999)
    fill(tree, '{pName}_sip3ds'.format(pName=pName), signedSip3D(lepton) if abs(lepton.pdgId())==11 or lepton.sourcePtr().innerTrack().isNonnull() else -9999)
    fill(tree, '{pName}_ip3ds'.format(pName=pName),  signedIp3D(lepton) if abs(lepton.pdgId())==11 or lepton.sourcePtr().innerTrack().isNonnull() else -9999 )
    fill(tree, '{pName}_dxy'.format(pName=pName), -lepton.dxy() if abs(lepton.pdgId())==11 or lepton.sourcePtr().innerTrack().isNonnull() else -9999 )
    fill(tree, '{pName}_dz'.format(pName=pName), lepton.dz() if abs(lepton.pdgId())==11 or lepton.sourcePtr().innerTrack().isNonnull() else -9999 )    
    ...
    if abs(lepton.pdgId())==13:
        ...
        fill(tree, '{pName}_innerHits'.format(pName=pName), lepton.sourcePtr().innerTrack().numberOfValidHits() if lepton.sourcePtr().innerTrack().isNonnull() else -9999)
        fill(tree, '{pName}_tightId'.format(pName=pName), lepton.tightId())
        fill(tree, '{pName}_tightFakeId'.format(pName=pName), lepton.tightFakeId())
        fill(tree, '{pName}_looseFakeId'.format(pName=pName), lepton.looseFakeId())
        fill(tree, '{pName}_edxy'.format(pName=pName), lepton.sourcePtr().edB() if lepton.sourcePtr().innerTrack().isNonnull() else -9999)
        fill(tree, '{pName}_edz'.format(pName=pName), lepton.sourcePtr().innerTrack().dzError() if lepton.sourcePtr().innerTrack().isNonnull() else -9999)
        fill(tree, '{pName}_pxb1'.format(pName=pName), lepton.sourcePtr().innerTrack().hitPattern().hasValidHitInFirstPixelBarrel() if lepton.sourcePtr().innerTrack().isNonnull() else -9999)
        fill(tree, '{pName}_pxlay'.format(pName=pName), lepton.sourcePtr().innerTrack().hitPattern().pixelLayersWithMeasurement() if lepton.sourcePtr().innerTrack().isNonnull() else -9999)
    ...
    if abs(lepton.pdgId()) == 13:
        if hasattr(lepton.sourcePtr(), 'innerTrack()'):
            fill(tree, '{pName}_tightCharge'.format(pName=pName),lepton.sourcePtr().innerTrack().ptError()/lepton.sourcePtr().innerTrack().pt() < 0.2)
